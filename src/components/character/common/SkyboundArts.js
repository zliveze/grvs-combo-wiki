...
// Hàm helper (có thể đã tồn tại ở NormalMove.js hoặc UniqueAction.js, cân nhắc tách ra file riêng)
...
// Card chung cho mỗi chiêu (SBA/SSBA)
const SkyboundArtCard = ({ art, inputGuide, initialActiveTab }) => {
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 md:p-4 hover:shadow-md transition-shadow duration-200">
      <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
          ...
          <div className="sm:w-1/4 flex flex-col items-center sm:items-start flex-shrink-0">
            ...
            {currentActiveTabData && currentActiveTabData.urls.length > 0 ? (
              currentActiveTabData.urls.map((url, index) => (
                <img
                  key={index} src={url} alt={`${art.name || 'Skybound Art'} ${activeImageTab} ${index + 1}`}
        ...
        <div className={`sm:w-${art.imageTabs && art.imageTabs.length > 0 ? '3/4' : 'full'} flex flex-col`}>
          <h4 className="text-xl md:text-2xl font-bold text-yellow-500 dark:text-yellow-400 mb-1">
            {art.name} <span className="text-sm font-normal text-gray-500 dark:text-gray-400">({art.type || 'Skybound Art'})</span>
          </h4>
...
const SkyboundArts = ({ skyboundArtsData, inputGuide }) => {
  const [isOpen, setIsOpen] = useState(true);

  if (!skyboundArtsData || skyboundArtsData.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow mt-6">
        <h3 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">Skybound Arts</h3>
        <p className="text-gray-700 dark:text-gray-300">Thông tin về Skybound Arts đang được cập nhật.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow mt-6">
      ...
      >
        <h3 className="text-2xl font-bold">Skybound Arts</h3>
        <svg
      ...
      {isOpen && (
        <div className="p-4 md:p-6 border-t border-gray-200 dark:border-gray-700">
          <div className="space-y-8">
            {skyboundArtsData.map((art) => (
              <SkyboundArtCard key={art.id || art.name} art={art} inputGuide={inputGuide} />
            ))}
          </div>
        </div>
      )}
